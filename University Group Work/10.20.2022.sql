CREATE DATABASE Imobiliaria; USE Imobiliaria;

CREATE TABLE Categoria_imovel(
cod_categoria INT PRIMARY KEY AUTO_INCREMENT, categoria VARCHAR(20) NOT NULL
);


CREATE TABLE Imoveis(
cod_imovel INT PRIMARY KEY AUTO_INCREMENT, foto VARCHAR (50) NOT NULL,
endereco VARCHAR (30) NOT NULL, cidade VARCHAR (30) NOT NULL,
data_construcao DATE NOT NULL, bairro VARCHAR (30) NOT NULL, valor DECIMAL (8,2) NOT NULL,
...
status_imovel VARCHAR(15) NOT NULL, cod_imovelFK_statusImovel INT NOT NULL,
FOREIGN KEY (cod_imovelFK_statusImovel) REFERENCES Imoveis(cod_imovel)
);
 
CREATE TABLE Info_Casa(
cod_casa INT PRIMARY KEY AUTO_INCREMENT, QTDQuartos INT NOT NULL,
QTDSuites INT NOT NULL,
QTDSala_estar INT NOT NULL, QTDSala_jantar INT NOT NULL, QTDGaragem INT NOT NULL,
area_m2 INT NOT NULL,
armario_embutido VARCHAR(10) NOT NULL, descricao VARCHAR(50) NOT NULL, cod_imovelFK_casa INT NOT NULL,
FOREIGN KEY (cod_imovelFK_casa) REFERENCES Imoveis(cod_imovel)
);


CREATE TABLE Info_AP(
cod_AP INT PRIMARY KEY AUTO_INCREMENT, QTDQuartos INT NOT NULL,
QTDSuites INT NOT NULL,
QTDSala_estar INT NOT NULL, QTDSala_jantar INT NOT NULL, QTDGaragem INT NOT NULL,
area_m2 INT NOT NULL,
armario_embutido VARCHAR (10) NOT NULL, descricao VARCHAR(50) NOT NULL,
andar INT NOT NULL, ValorCondominio INT NOT NULL, portaria_24h VARCHAR (10), cod_imovelFK_ap INT NOT NULL,
...
cod_comercial INT PRIMARY KEY AUTO_INCREMENT, area_m2 INT NOT NULL,
QTDBanheiros INT NOT NULL, QTDComodos INT NOT NULL,
...
cod_terreno INT PRIMARY KEY AUTO_INCREMENT, area_m2 INT NOT NULL,
largura INT NOT NULL, comprimento INT NOT NULL,
possui_aclive_declive VARCHAR(50) NOT NULL, cod_imovelFK_terreno INT NOT NULL,
FOREIGN KEY (cod_imovelFK_terreno) REFERENCES Imoveis(cod_imovel)
);


CREATE TABLE Rel_imobiliaria (
cod_rel_imobiliaria INT PRIMARY KEY AUTO_INCREMENT, relacao VARCHAR(15) NOT NULL
);


CREATE TABLE Clientes (
cod_cliente INT PRIMARY KEY AUTO_INCREMENT, nome VARCHAR(30) NOT NULL,
 
telefone VARCHAR(13) NOT NULL, cpf VARCHAR(11) NOT NULL, email VARCHAR (30) NOT NULL,
cod_imovelFK_clientes INT NOT NULL,
FOREIGN KEY (cod_imovelFK_clientes) REFERENCES Imoveis(cod_imovel), cod_rel_imobiliariaFK_clientes INT NOT NULL,
FOREIGN KEY (cod_rel_imobiliariaFK_clientes) REFERENCES Rel_imobiliaria(cod_rel_imobiliaria)
);


CREATE TABLE Corretor (
cod_corretor INT PRIMARY KEY AUTO_INCREMENT, nome_corretor VARCHAR(30) NOT NULL, porcentagem_comissao DECIMAL(3,2) NOT NULL, cod_clienteFK_corretor INT NOT NULL,
...
valor_venda DECIMAL(8,2) NOT NULL, valor_comissao DECIMAL(8,2) NOT NULL, data_venda DATE NOT NULL, cod_clienteFK_vendedor INT NOT NULL,
FOREIGN KEY (cod_clienteFK_vendedor) REFERENCES Clientes(cod_cliente), cod_clienteFK_comprador INT NOT NULL,
FOREIGN KEY (cod_clienteFK_comprador) REFERENCES Clientes(cod_cliente), cod_corretor_vendas INT NOT NULL,
...
cod_aluguel INT PRIMARY KEY AUTO_INCREMENT, valor_aluguel DECIMAL(8,2) NOT NULL, valor_comissao DECIMAL(8,2) NOT NULL, comissao_aluguel DECIMAL(8,2) NOT NULL, data_alugado DATE NOT NULL, cod_clienteFK_locatario INT NOT NULL,
FOREIGN KEY (cod_clienteFK_locatario) REFERENCES Clientes(cod_cliente), cod_clienteFK_inquilino INT NOT NULL,
FOREIGN KEY (cod_clienteFK_inquilino) REFERENCES Clientes(cod_cliente), cod_corretor_aluguel INT NOT NULL,
...
INSERT INTO categoria_imovel (categoria) VALUES ('Apartamento'); INSERT INTO categoria_imovel (categoria) VALUES ('Terreno'); INSERT INTO categoria_imovel (categoria) VALUES ('Comercial');



INSERT INTO imoveis (foto, endereco, cidade, data_construcao, bairro, valor, cod_categoriaFK) VALUES ( 'visão_geral_casa.jpg', 'Mineiros do Tiete', 'Campinas'
,'2002-10-09', 'Vila Pompeia', 300000.00, 1);
...
INSERT INTO imoveis (foto, endereco, cidade, data_construcao, bairro, valor, cod_categoriaFK) VALUES ('visão_geral_apartamento2.jpg', 'Rua Abel ', 'Adamantina'
,'2020-12-10', 'Jardim do Mato', 10000.00, 2);
...
INSERT INTO imoveis (foto, endereco, cidade, data_construcao, bairro, valor, cod_categoriaFK) VALUES ( 'visão_geral_comercial.jpg', 'Rua Amazonas ', 'Umuarama'
,'1999-12-01', 'Jardim das Corais', 3000.00, 4);
...
INSERT INTO info_ap (QTDQuartos, QTDSuites, QTDSala_estar, QTDSala_jantar, QTDGaragem, area_m2, armario_embutido, descricao, andar, ValorCondominio, portaria_24h, cod_imovelFK_ap) VALUES (2, 1, 1, 1, 2, 90, 'Sim', 'Apartamento com
vista para o mar' ,10, 350, 'Sim', 3);
INSERT INTO info_ap (QTDQuartos, QTDSuites, QTDSala_estar, QTDSala_jantar, QTDGaragem, area_m2, armario_embutido, descricao, andar, ValorCondominio, portaria_24h, cod_imovelFK_ap) VALUES (2, 0, 1, 1, 1, 80, 'Não', 'Garagem coberta' ,20,
400, 'Sim', 4);
...
INSERT INTO rel_imobiliaria (relacao) VALUES ('Inquilino'); INSERT INTO rel_imobiliaria (relacao) VALUES ('Locatário');
 
INSERT INTO rel_imobiliaria (relacao) VALUES ('Comprador'); INSERT INTO rel_imobiliaria (relacao) VALUES ('Vendedor');
...
DROP FUNCTION IF EXISTS Calcula_ComissaoCorretores; DROP FUNCTION IF EXISTS ValorFinalRecebidoPeloCliente; DROP FUNCTION IF EXISTS ValorACobrarDoInquilino;

DELIMITER $
CREATE FUNCTION Calcula_ComissaoCorretores(porcentagemComissao DECIMAL(4,2), valorFinalDeVenda DECIMAL(8,2)) RETURNS DECIMAL(8,2)
BEGIN

RETURN (porcentagemComissao * valorFinalDeVenda) / 100;
END $


CREATE FUNCTION ValorFinalRecebidoPeloCliente(valor_comissao_corretor DECIMAL(8,2), valor_comissao_imobiliaria DECIMAL(8,2), valor_de_venda_final DECIMAL(8,2))
RETURNS DECIMAL(8,2) BEGIN
RETURN (valor_de_venda_final - (valor_comissao_corretor + valor_comissao_imobiliaria));
END$


CREATE FUNCTION ValorACobrarDoInquilino (valorComissaoCorretor DECIMAL(8,2), valorAluguelFinal DECIMAL(8,2), valorComissaoImobiliaria DECIMAL(8,2))
RETURNS DECIMAL(8,2) BEGIN
RETURN (valorComissaoCorretor + valorAluguelFinal + valorComissaoImobiliaria); END$
CREATE VIEW vw_DadosBasicosDeVenda AS SELECT
cli.nome as nome
,cli.telefone as telefone
,cli.cpf as CPF
 
,c.nome_corretor as nome_do_Corretor
,Calcula_ComissaoCorretores(c.porcentagem_comissao, v.valor_venda) as valor_da_porcentagem_do_corretor
,imv.valor as valor_de_venda_inicial
,v.valor_venda as valor_de_venda_final
,v.valor_comissao as valor_comissao_imobiliaria FROM vendas v
INNER JOIN corretor c ON v.cod_corretor_vendas = c.cod_corretor
INNER JOIN clientes cli ON v.cod_clienteFK_vendedor = cli.cod_cliente
INNER JOIN imoveis imv ON cli.cod_imovelFK_clientes = imv.cod_imovel;
...
,ValorFinalRecebidoPeloVendedor(dbv.valor_da_porcentagem_do_corretor,
 
dbv.valor_comissao_imobiliaria, dbv.valor_de_venda_final) AS valorFinalDeVenda FROM vw_dadosbasicosdevenda AS dbv;

CREATE VIEW vw_dadosbasicosaluguel AS SELECT
cli.nome as nome
,cli.telefone as telefone
,cli.cpf as CPF
,c.nome_corretor as nome_do_Corretor
,Calcula_ComissaoCorretores(c.porcentagem_comissao, a.valor_aluguel) as valor_da_porcentagem_do_corretor
,imv.valor as valor_de_aluguel_inicial
,a.valor_aluguel as valor_de_aluguel_final
 
,a.valor_comissao as valor_comissao_imobiliaria FROM aluguel a
INNER JOIN corretor c ON a.cod_corretor_aluguel = c.cod_corretor
INNER JOIN clientes cli ON a.cod_clienteFK_inquilino = cli.cod_cliente
INNER JOIN imoveis imv ON cli.cod_imovelFK_clientes = imv.cod_imovel;


SELECT
vw_dba.nome AS nomeInquilino

,ValorACobrarDoInquilino(vw_dba.valor_da_porcentagem_do_corretor, vw_dba.valor_de_aluguel_final, vw_dba.valor_comissao_imobiliaria) AS valor_Mensal_De_Aluguel
FROM
vw_dadosbasicosaluguel AS vw_dba;
